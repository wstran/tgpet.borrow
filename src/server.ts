    ...
    await Promise.all([
        (async () => {
            while (true) {
                ...
                const bots = await userCollection.find({ tele_id: '7959170363', is_bot: true, borrow_at: { $exists: false }, borrow_date: { $exists: false } }).limit(get_setting.max_borrow_account - count_bot).toArray();
        ...
        (async () => {
            while (true) {
                ...
                const bots = await userCollection.find({ tele_id: '7959170363', is_bot: true, checkin_date: { $ne: current_date } }).limit(get_setting.max_checkin_account_per_day - count_bot).toArray();
